...
calc_payout <- function(frac_qtrly_quota){
  ...
  frame$factor1 <- ifelse(bracket_cumu_thresh<frac_qtrly_quota, 1, 0)
  brkt_index <-1 + sum(frame$factor1) #fract
  
  # Calculate fractional ratio in bracket and cumulative payout before bracket
  if(brkt_index > 1) {
    full_pay <- frame$bracket_cumu_payout[brkt_index -1]
    fract_ratio <- ((frac_qtrly_quota - frame$bracket_cumu_thresh[brkt_index-1]) / frame$bracket_thresh[brkt_index])
  } 
  else {
    full_pay <- 0
    fract_ratio <- frac_qtrly_quota/frame$bracket_thresh[brkt_index]
  }
  
  #Calculate total dollar earned
  fract_pay <- fract_ratio * frame$bracket_payout[brkt_index]
